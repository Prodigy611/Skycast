 ...
 * The OWM_API_KEY environment variable is never exposed to the browser.
 *
 * Endpoints this function handles:
 *   GET /api/weather?type=current&lat=X&lon=Y
 *   GET /api/weather?type=forecast&lat=X&lon=Y
 *   GET /api/weather?type=geo&city=Lagos
 */
...
exports.handler = async (event) => {
  ...
  if (!key) {
    return { statusCode: 500, headers, body: JSON.stringify({ error: 'Weather API key not configured on server.' }) };
  }
  ...
  try {
    ...
    if (type === 'geo' && city) {
      ...
      return { statusCode: 400, headers, body: JSON.stringify({ error: 'Invalid request parameters.' }) };
    }
    ...
    if (!res.ok) {
      return { statusCode: res.status, headers, body: JSON.stringify({ error: data.message || 'OpenWeatherMap error' }) };
    }
